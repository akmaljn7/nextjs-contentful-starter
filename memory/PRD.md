...
- **Admin** approve/deny is unchanged — handled on the web console (`/admin/time-off`); approved leave already overrides the schedule and blocks attendance on covered dates (`sessions.py`).
- **No backend changes** — reuses existing endpoints (`POST /time-off`, `GET /time-off/me`, `DELETE /time-off/{id}`, admin `PATCH /approve|/deny`).
- **Verified**: `yarn typecheck` clean; full flow curl e2e (employee create → pending → admin approve w/ notes → employee sees approved → delete). ⚠️ The mobile screen UI is **not renderable in-container** (React Native) — confirm on device after an **APK/IPA rebuild**.

### Reliability batch: Wake Diagnostics + Geofence Lattice + Android Re-arm (June 2026)
- **1) Wake Diagnostics (admin visibility)** — every background wake now records *what* woke the app (`geofence` / `geofence_ring` / `slc` / `visit` / `boot` / `rearm`). Mobile: `src/services/wakeLog.ts` posts to new `POST /api/mobile/wake`; called from the geofence task, SLC + CLVisit (`backgroundResurrect(source)`), the ring branch, boot task, and the re-arm task. Backend: `MobileWake` model + `device_wakes` collection (indexed `org_id+user_id+ts_ms`, 14-day TTL), `POST /wake` (any auth) and admin `GET /wakes` (org-scoped, returns rows + per-employee source-count summary). Web: new **WAKE ACTIVITY** panel on `AdminDashboard.jsx` (`data-testid=wake-activity`, per-employee source badges + "last woke" — auto-refresh 20s). **Verified in-container**: full e2e curl (employee posts wakes → admin reads rows + summary; 401 unauth) + admin screenshot showing the panel with `iOS VISIT · 1` / `APPROACH RING · 1` badges.
- **2) Geofence Lattice** — `registerOfficeGeofence` now arms TWO concentric regions: the precise **core** (`office.id`, attendance crossings) + an **outer approach ring** (`office.id::ring`, radius = min(2000, max(150, radius×3))). The geofence task treats a `::ring` crossing as **wake-only** (re-arm geofences + capture a fix + drain via `backgroundResurrect(..., "geofence_ring")`) — never an attendance enter/exit — so the app wakes/re-arms as the employee nears the office, earlier than the core boundary or an SLC tower change. iOS 20-region budget: 2/office.
- **3) Android periodic Re-arm** — added `expo-background-fetch@13.0.6` (WorkManager). New `src/services/reArm.ts` defines `gfattend.rearm` (min 15-min interval, `stopOnTerminate:false`, `startOnBoot:true`) that rebuilds the whole stack (reconcile → sync geofence → start live location → drain queues → heartbeat), so a **swiped-away** Android app recovers without waiting for a reboot. Registered/unregistered in `AuthContext` (employee login/bootstrap → register; sign-out → unregister); task imported at top-level in `App.tsx`. iOS rides BGTask as a bonus (SLC/CLVisit cover force-quit there).
- **Verified**: backend curl e2e ✅, web panel screenshot ✅, `yarn typecheck` clean ✅, CLVisit module still autolinks ✅. ⚠️ Native behaviours (lattice crossings, WorkManager re-arm, on-device wake sources) are **not testable in-container** — confirm on physical devices. **Requires an APK/IPA rebuild.**
...
- **Fix (`mobile/src/services/geofence.ts`)**: added `SLC_TASK = "gfattend.slc"` constant (replacing string literals in start/stop) and a top-level `TaskManager.defineTask(SLC_TASK, ...)` that on each wake: (1) calls `syncOfficeGeofence()` to re-arm CLCircularRegions, (2) enqueues each SLC `LocationObject` as an offline-durable location fix (with battery piggyback, per user choice to treat SLC fixes as real location updates), (3) drains both queues — `drainLocationQueue()` (POSTs to `/api/mobile/location-sync`, driving the server session state machine auto-start/pause/resume, idempotent via `last_live_ts_ms` watermark) and `drainQueue()` (flushes buffered enter/exit crossings via `/api/mobile/sync`).
- **No backend changes** — reuses existing bulk sync endpoints; server idempotency makes SLC replays safe.
- **Verified**: `yarn typecheck` clean (0 errors). ⚠️ Real SLC/force-quit resurrection behaviour is iOS-only and **not testable in-container** — user confirms on physical iPhone / TestFlight build. **Requires an APK/IPA rebuild.**
...
- **[16 Jun 2026]** Five reported issues fixed:
  1. **(backend, verified 8/8 — iteration_32) Proxy check-in went offline:** when a lending phone checked in a 2nd colleague, the 1st dropped to "offline". Root cause: `/api/mobile/location` only refreshed the streaming user's OWN session, so `source='proxy_checkin'` sessions never got a live-freshness bump. Fix (`routes/mobile.py` ~L571): each live fix now `update_many`s `last_live_ts_ms`+`last_fix` on all active sessions where `proxy_by == streamer email`, so every colleague proxied by one phone stays online together.
  2. **(mobile) "Welcome to the office" spam:** iOS re-fires geofence ENTER on every app open / geofence re-register, spamming the notice while already inside. Fix (`services/geofence.ts`): persist presence in AsyncStorage (`gfattend.presence`); only notify on a real inside↔outside flip.
  3. **(mobile) Offline detection notification:** geofence EXIT keeps the original "⏸ Attendance paused / See you next time." The separate "⚠️ You are offline" alert now fires from `services/connectivity.ts` when the device stays without internet for >12s (debounced against blips), with a "✅ Back online" follow-up on reconnect — mirroring the admin console's offline state.
  4. **(mobile) Label rename:** HealthChip green state "TRACKING ACTIVE" → "YOU ARE IN OFFICE".
  5. **(mobile) Admin create-office crash — Rules of Hooks violation:** `OfficeEditModal` called `useMemo` AFTER an early `return null`, so opening the modal (draft null→set) changed the hook count and React hard-crashed. Fix (`screens/admin/OfficesScreen.tsx`): moved all hooks above the early return.
  - #2–#5 are React Native native/UI and are **not testable by the automated agent** — confirm on BrowserStack/device. tsc clean.
  ...
  - **iOS software-keyboard fix (the real login bug):** After the reanimated removal the login field STILL didn't accept typing on a physical iPhone — but on BrowserStack using the **hardware/computer keyboard** login worked. That isolated it to the **on-screen keyboard presentation**. Root cause: `LoginScreen` nested a `KeyboardAvoidingView` (behavior `padding`) INSIDE the scrollable `Screen`'s `ScrollView` — a documented iOS anti-pattern that makes the software keyboard flicker open/closed so its keys can't be tapped (a hardware keyboard never triggers that presentation, hence it worked on BrowserStack). Fix: removed the nested `KeyboardAvoidingView` and added `automaticallyAdjustKeyboardInsets` to the `ScrollView` in `Screen.tsx` (native iOS keyboard avoidance, no flicker). Only `LoginScreen` used KAV. tsc clean. **Must be confirmed on BrowserStack/device by tapping the on-screen keyboard.**
